...
async function main() {
	...
	/* ------------- */
	const frankenstein = await prisma.pub.create({
		data: {
			pubTypeId: typeIds[0],
			communityId: communityUUID,
			values: {
			...
			values: {
				createMany: {
					data: [
						...
						{ fieldId: fieldIds[6], value: frankenstein.id },
					],
				},
			},
		},
	...
	/* HDS */
	const hdsr = await prisma.pub.create({
		data: {
			pubTypeId: typeIds[2],
			communityId: communityUUID,
			values: {
			...
			values: {
				createMany: {
					data: [
						...
						{ fieldId: fieldIds[6], value: hdsr.id },
					],
				},
			},
		},
