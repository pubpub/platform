 ...
 */
const RelationConfigSection = ({ relationFields, pubTypeId }: RelationConfigSectionProps) => {
	...
	// Fetch pub type info to get field's relatedPubTypes
	const { data: pubTypeData } = client.site.pubTypes.get.useQuery({
		queryKey: ["pubType", pubTypeId],
		queryData: {
			params: { communitySlug: community.slug, pubTypeId },
		},
		enabled: !!pubTypeId,
	})

	// Extract relatedPubTypes from the selected field in the fetched pub type
	const relatedPubTypeIds = useMemo(() => {
		if (!pubTypeData?.body?.fields || !selectedFieldSlug) return undefined
		const field = pubTypeData.body.fields.find(
			(f: { slug: string }) => f.slug === selectedFieldSlug
		)
		// The field may have relatedPubTypes from form elements
		return selectedField?.relatedPubTypes
	}, [pubTypeData, selectedFieldSlug, selectedField])

	// State for JSONata mode toggles
	const [pubIdMode, setPubIdMode] = useState<"normal" | "jsonata">("normal")
	const [valueMode, setValueMode] = useState<"normal" | "jsonata">("normal")

	// Check if existing values are JSONata expressions
	useEffect(() => {
		const relatedPubId = form.getValues(`${basePath}.relatedPubId`)
		const value = form.getValues(`${basePath}.value`)
		if (isJsonTemplate(relatedPubId)) {
	...
	return (
		<FieldSet className="mt-4 rounded-md border border-gray-200 p-4">
			...
			{selectedFormIsLoading ? (
				...
				<>
					...
					<RelationConfigSection
						...
						pubTypeId={selectedForm.body.pubTypeId}
					/>
				</>
			) : null}
		</FieldSet>
