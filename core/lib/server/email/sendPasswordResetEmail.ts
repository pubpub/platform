...
export class Email {
	...
	constructor(
		public from: string = `hello@pubpub.org`,
		public name: string = `PubPub Team`
	) {}

	#buildSend(emailPromise: () => Promise<RequiredOptions>) {
	...
	async #send(
		...
		try {
			...
			const send = await smtpclient.sendMail({
				from: `${options?.name ?? this.name} <${options?.from ?? this.from}>`,
				to: required.to,
