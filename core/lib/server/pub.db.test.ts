...
describe("getPubsWithRelatedValues", () => {
	...
	it("should filter values by form", async () => {
		const trx = getTrx();
		const { seedCommunity } = await import("~/prisma/seed/seedCommunity");
		// Make a new seed that has a more limited default form
		const seed = createSeed({
			community: {
				name: "test",
				slug: "test-form-view",
			...
			pubs: [
				{
					...
					members: { contributor: MemberRole.contributor },
				},
				{
					pubType: "Basic Pub",
					values: {
		...
		const { community, pubs, users, pubTypes, forms } = await seedCommunity(seed);
		const { getPubsWithRelatedValues } = await import("./pub");

		// Test the builtin default form first which only has 'Title' and 'Some Relation'
		const pub1 = await getPubsWithRelatedValues(
			{ pubId: pubs[0].id, communityId: community.id },
			{ withValues: true }
		);
		...
		expect(pub1.values).toMatchObject([
			...
			// This value only exists on the pub, not the form
			{
		...
		// Swap out the default form for basic-pub-form
		await trx
			.updateTable("forms")
			.set({ isDefault: false })
			.where("forms.pubTypeId", "=", pubTypes["Basic Pub"].id)
			.execute();
		await trx
			.updateTable("forms")
			.set({ isDefault: true })
			.where("forms.id", "=", forms["basic-pub-form"].id)
			.execute();

		// For a contributor, should be limited to just Title
		// should be able to optionally specify a form to use as the 'filter'?
		const pub = await getPubsWithRelatedValues(
			{ pubId: pubs[0].id, communityId: community.id },
			{ withValues: true }
		);
		expect(pub.values).toHaveLength(2);
		expect(pub.values).toMatchObject([
			// Should order by form element rank
			{
				value: "Some description",
				relatedPubId: null,
				schemaName: "String",
				fieldName: "Description",
				formElementId: forms["basic-pub-form"].elements[0].id,
			},
			{
				value: "Some title",
				relatedPubId: null,
				schemaName: "String",
				fieldName: "Title",
				formElementId: forms["basic-pub-form"].elements[1].id,
			},
		]);
	});
});
