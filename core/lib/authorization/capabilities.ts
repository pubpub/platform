...
} & XOR<{ slug: string; communityId: CommunitiesId }, { pubId: PubsId }>;
...
export const whereIdOrSlug = <EB extends ExpressionBuilder<any, any>, T extends string>(
	eb: EB,
	table: string,
	idOrSlug: XOR<{ slug: string; communityId: CommunitiesId }, { [K in IdLike]: string }>
) => {
	if ("slug" in idOrSlug) {
		return eb.and([
			eb(`${table}.slug`, "=", idOrSlug.slug),
			eb(`${table}.communityId`, "=", idOrSlug.communityId),
		]);
	}
		...
		const capabilitiesQuery = db
			...
			.with("community_ms", (db) =>
				db
					...
					.where((eb) =>
						eb("communityId", "=", eb.selectFrom("pub").select("communityId"))
					)
					.select("role")
			)
