...
describe("interpolate", () => {
	...
	describe("error cases", () => {
		...
		test("throws on multiple raw interpolations", async () => {
			await expect(async () => {
				await interpolate("{{ $.a }}{{ $.b }}", { a: 1, b: 2 });
			}).rejects.toThrow(
				"multiple interpolations are only allowed in string contexts, found 2 interpolations in non-string template"
			);
		});

		test("throws on multiple interpolations with spaces", async () => {
			await expect(async () => {
				await interpolate("{{ $.a }} {{ $.b }}", { a: 1, b: 2 });
			}).rejects.toThrow("multiple interpolations are only allowed in string contexts");
		});
	...
	describe("edge cases", () => {
		...
		test("handles JSONata arithmetic", async () => {
			const result = await interpolate("{{ $.a + $.b }}", { a: 5, b: 3 });
